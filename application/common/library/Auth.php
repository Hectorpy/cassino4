...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'payerFirstName','payerLastName','payerEmail','payerPhone','payerCPF','nickname', 'level','mobile', 'avatar','money', 'score','stat','pid'];
    ...
    public function getUserinfo()
    {
        ...
        // dump($userinfo);die;
        $vips = db::name("vips")->where("level", $userinfo['level'])->find();
        ...
        $userinfo['recharge'] = round($vips['total_recharge'], 2) >= round($vips['running_money'], 2)?
        round($vips['running_money'], 2)."/".round($vips['running_money'], 2):
        round($vips['total_recharge'], 2)."/".round($vips['running_money'], 2);
        $userinfo['bet'] = round($vips['total_bet'], 2) >= round($vips['running_water'], 2)?
        ...
        $userinfo['recharge_b'] = $recharge_b >= 100?100: $recharge_b;
        $userinfo['bet_b'] = $bet_b >= 100?100: $bet_b;
